...
@section('content')
  ...
  <div class="span9">
	<!--
		...
		-->
<?php
	echo Breadcrumb::create(array( 'Add' => $url.'/admin/movie/add', 'List' => $url.'/admin/movie/list', 'Edit'));

  	if ( isset( $movie ) ) {				// Check if movie is givven!
		foreach ( $movie as $uniqValue ) {
...
<?php 
	...
	echo Form::horizontal_open();
		...
		echo Form::control_group(
			...
			Form::block_help('choose your path'));

		$rt = $uniqValue->rate ;
		echo Form::control_group(
			Form::label('rt', 'Rate'), 
			Form::medium_text('rt', $rt), '', 
			Form::block_help('choose your rate'));
